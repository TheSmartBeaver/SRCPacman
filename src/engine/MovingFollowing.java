...
public class MovingFollowing extends MovingStrategy {
    ...
    public void computeInputList(Ghost ghost) {
        ...
        List<TileScore> open = null, close = null;
        ...
        while(!open.isEmpty()){
            ...
            for (TileScore possibleTile : possiblesTiles){
                ...
                for(int i = 0; i<open.size(); ++i){
                    if(possibleTile.getPosX() == open.get(i).getPosX() && possibleTile.getPosY() == open.get(i).getPosY() && possibleTile.getScore() > open.get(i).getScore()) {
                        skip = true;
                        break;
                    }
                }
                ...
                for(int i = 0; i<close.size(); ++i){
                    if(possibleTile.getPosX() == open.get(i).getPosX() && possibleTile.getPosY() == open.get(i).getPosY() && possibleTile.getScore() > close.get(i).getScore()) {
                        skip = true;
                        break;
                    }
                }
